import * as service from '../../services/rcontrol/alarmConfig';

const namespace = 'alarmConfig';
const pathname = '/rcontrol/alarm-config';
...
export default {
  ...
  effects: {
    ...
    *add({ data, callback }, { call, put, select }) {
      const response = yield call(service.add, data);
    ...
    *upt({ data, callback }, { call, put, select }) {
      const response = yield call(service.upt, data);
      if (response && response.code === 200) {
        if (callback) callback();
        const pagination = yield select(state => {
          return state[namespace].list.pagination;
    ...
    // 查询单条数据
    *get({ pid }, { call, put }) {
      const response = yield call(service.get, pid);
      if (response && response.code === 200 && response.body) {
        yield put({
          type: 'setEditTarget',
    ...
    *del({ pid, callback }, { call, put, select }) {
      const response = yield call(service.del, pid);
      if (response && response.code === 200) {
        if (callback) callback();
        const pagination = yield select(state => {
          return state[namespace].list.pagination;
